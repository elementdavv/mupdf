	...
	while (node)
	{
		...
		case FLOW_WORD:
			...
			if (line_w + node->w > box->s.layout.w && !candidate)
			{
				...
				{
					break_word_for_overflow_wrap(ctx, node, ld, box->s.layout.w - line_w);
				}
			}
...
static void layout_table(fz_context *ctx, layout_data *ld, fz_html_box *box, fz_html_box *top)
{
	...
	fz_try(ctx)
	{
		...
		/* The table can fit within the available space.
		 * And the maximum table width may be greater or smaller than the available width
		 * In this case, scale the collumns to fill the space.
		 * Take into account when column width becomes smaller than minimum width after scaled
		 */
		else {
			float avail_w = box->s.layout.w - spacing * (ncol + 1);
			max_tabw -= spacing * (ncol + 1);
			float scale_w = avail_w / max_tabw;
			float poor = 0.0f, rich = 0.0f;
			for (col = 0; col < ncol; ++col)
			{
				colw[col].actual = colw[col].max * scale_w;
				if (colw[col].actual < colw[col].min)
				{
					poor += colw[col].min - colw[col].actual;
					colw[col].actual = colw[col].min;
				}
				else if (colw[col].actual - colw[col].min > 1.0f)
				{
					rich += colw[col].actual;
				}
			}
			float scale_aid = (rich - poor) / rich;
			for (col = 0; col < ncol; ++col)
			{
				if (colw[col].actual - colw[col].min > 1.0f)
				{
					colw[col].actual *= scale_aid;
				}
			}
		}
